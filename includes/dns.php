...
class DNS
{
    ...
    public static function ProcessNSUpdateForDomain($input, $domain)
    {
        global $g_domains;
        ...
        if (!array_key_exists("explicit", $domain_info) || $domain_info["explicit"] === true)
            $zone_name = $domain;
        Common::IncrementStat('nsupdate_call');
        return nsupdate($input, $tsig, $tsig_key, $zone_name);
    }

    public static function SplitLongTXTValue($value)
    {
        $trimmed = trim($value);
        if ($trimmed === '')
            return $value;
    ...
    public static function ProcessInsertFromPOST($zone, $record, $value, $type, $ttl)
    {
        ...
        if ($type == 'TXT' && $g_auto_split_long_txt)
            $value = self::SplitLongTXTValue($value);

        if (!Validator::NSupdateEscapeCheck($fqdn))
            Error('Invalid data for record name: ' . $fqdn);
        if (!Validator::NSupdateEscapeCheck($ttl))
            Error('Invalid data for TTL: ' . $ttl);
        if (!Validator::NSupdateEscapeCheck($type))
            Error('Invalid data for record type: ' . $type);
        if (!Validator::NSupdateEscapeCheck($value))
    ...
    public static function CreateRecord($zone, $record, $value, $type, $ttl, $comment)
    {
        global $g_domains;
        $input = "server " . $g_domains[$zone]['update_server'] . "\n";
        $input .= self::ProcessInsertFromPOST($zone, $record, $value, $type, $ttl);
        $input .= "send\nquit\n";
        $result = self::ProcessNSUpdateForDomain($input, $zone);
        if (strlen($result) > 0)
            Debug("result: " . $result);
        Audit::Write('create', $record . "." . $zone . " " . $ttl . " " . $type . " " . $value, $comment);
        Common::IncrementStat('create');
        return true;
    }

    //! Replace record - atomic, returns true on success
    public static function ModifyRecord($zone, $record, $value, $type, $ttl, $comment, $old, $is_fqdn = false)
    {
        global $g_domains;
        if (!Validator::NSupdateEscapeCheck($old))
        ...
        if ($is_fqdn == false)
            ...
            $input .= self::ProcessInsertFromPOST(NULL, $record, $value, $type, $ttl);
        $input .= "send\nquit\n";
        $result = self::ProcessNSUpdateForDomain($input, $zone);
        if (strlen($result) > 0)
            Debug("result: " . $result);
        Audit::Write('replace_delete', $old, $comment);
        Common::IncrementStat('replace_delete');
        Audit::Write('replace_create', $record . "." . $zone . " " . $ttl . " " . $type . " " . $value, $comment);
        Common::IncrementStat('replace_create');
        return true;
    }

    public static function DeleteRecord($zone, $record)
    {
        global $g_domains;
        ...
        if (!Validator::NSupdateEscapeCheck($record))
            Error("Invalid delete string: " . $record);

        $input = "server " . $g_domains[$zone]['update_server'] . "\n";
        $input .= "update delete " . $record . "\nsend\nquit\n";
        self::ProcessNSUpdateForDomain($input, $zone);
        Audit::Write("delete", $record);
        Common::IncrementStat('delete');
        return true;
    }
    ...
    public static function InsertPTRForARecord($ip, $fqdn, $ttl, $comment)
    {
        global $g_domains;
        ...
        $input .= self::ProcessInsertFromPOST(NULL, $arpa, $fqdn, 'PTR', $ttl);
        $input .= "send\nquit\n";
        $result = self::ProcessNSUpdateForDomain($input, $arpa_zone);
        if (strlen($result) > 0)
            Debug("result: " . $result);
        Audit::Write('create', $arpa . " " . $ttl . " PTR " . $fqdn, $comment);
        Common::IncrementStat('create');
        return true;
    }
    ...
    public static function DeletePTRForARecord($ip, $fqdn, $comment)
    {
        global $g_domains;
        ...
        if (strlen($result) > 0)
            Debug("result: " . $result);
        Audit::Write('delete', $arpa . " 0 PTR " . $fqdn, $comment);
        Common::IncrementStat('delete');
        return true;
    }
}
