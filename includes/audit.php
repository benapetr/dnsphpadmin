...
class Audit
{
    public static function LogBatch($operation)
    {
    ...
    public static function Write($operation, $text = '', $comment = NULL)
    {
        ...
        $raw_line = 'entry point: ' . G_DNSTOOL_ENTRY_POINT . ' eid: ' . $g_eid . ' user: ' . Auth::GetCurrentUserName() . " ip: " . $_SERVER['REMOTE_ADDR'] . " operation: " . $operation . $record . $comment;

        if ($g_syslog && $g_syslog_targets['audit'] === true)
        {
            Logging::WriteToSyslog($raw_line);
        }
...
{
    return Audit::LogBatch($operation);
}

function WriteToAuditFile($operation, $text = '', $comment = NULL)
{
    return Audit::Write($operation, $text, $comment);
}
